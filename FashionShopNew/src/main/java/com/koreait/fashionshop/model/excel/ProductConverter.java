			...
			for(int i=1;i<rowCount;i++) {
				...
				for(int a=0;a<row.getPhysicalNumberOfCells();a++) {
					...
					if(a==0) {//subcategory_id
						...
						String[] colors=cell.getStringCellValue().trim().split(","); //공백을 제거하고, 점을 기준으로 나누면, 스트링 배열이반환!!
						...
						String[] psizes = cell.getStringCellValue().split(","); //점을 기준으로 나누면, 스트링 배열이반환!!
						List psizeList = new ArrayList();
						for(String psize :psizes) {
							Psize obj = new Psize();
							obj.setFit(psize);  //하나의 색상 vo에 색상정보를 대입
							psizeList.add(obj);
						}
