...
public class PaymentController {
	...
	public ModelAndView editCart(@RequestParam("cart_id") int [] cartarr, @RequestParam("quantity") int[] quantityarr) {
		
		MessageData messageData = new MessageData();
		ModelAndView mav = new ModelAndView();
		
		messageData.setResultCode(1);
		messageData.setMsg("수정 완료!");
		messageData.setUrl("/shop/cart/list");
		List<Cart> cartList = new ArrayList<Cart>();
		for(int i=0; i<cartarr.length; i++) {
			Cart cart = new Cart();
			cart.setCart_id(cartarr[i]);
		...
		//logger.debug("cart_id : " +  cartarr.length);
		//logger.debug("quantity : " + quantityarr.length);
